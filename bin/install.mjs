#!/usr/bin/env node
// install.mjs — installs dv-agents for the AI coding agents you
// actually have installed.
//
// Usage (via npm):
//   npx dv-agents                 # auto-detect + confirm (default)
//   npx dv-agents --yes           # auto-detect, no prompt
//   npx dv-agents --ide claude    # install one target explicitly
//   npx dv-agents --ide all       # install every supported target
//   npx dv-agents --ide gemini --global
//
...
function ensureDurablePayload() {
  ...
  const root = join(homedir(), ".dv-agents", "payload", version);
...
function installGemini(outPath, refRoot) {
  ...
  const lines = [
    ...
    "<!-- Generated by dv-agents installer (npm) -->",
    `<!-- Source: ${refRoot} -->`,
    "",
    header,
    "",
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === "--ide") {
      ...
      console.log(
        "Usage: npx dv-agents [--ide claude|copilot|gemini|opencode|codex|all] [--global] [--yes]\n" +
      ...
      fail(
        `Unknown argument: ${arg}\n` +
          `Usage: npx dv-agents [--ide <target>] [--global] [--yes]`
      );
    }
  }
  ...
  if (detected.length === 0) {
    console.log(
      "\nNo supported agents detected. Install one explicitly with:\n" +
        "  npx dv-agents --ide claude   (or copilot|gemini|opencode|codex|all)"
    );
    return;
  }
